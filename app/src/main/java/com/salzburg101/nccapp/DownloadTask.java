...
public class DownloadTask {
    ...
    public static final String downloadDirectory = "NCC_Downloads";

    private static ListenerSendAttachmentByEmail mListener;

    public static void setListenerSendAttachment(ListenerSendAttachmentByEmail eventListener) {
        mListener = eventListener;
    }

    public DownloadTask(Context context, Button buttonText, String downloadUrl, String fileName) {
        this.context = context;
        this.buttonText = buttonText;
        this.downloadUrl = downloadUrl;

        // downloadFileName = downloadUrl.replace(mainUrl, "");//Create file name by picking download file name from URL
        downloadFileName = fileName; //Create file name by using name included in the method call.
    ...
    private class DownloadingTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void result) {
            try {
                if (outputFile != null) {
                    ...
                    if (GlobalVariables.sendAttachment == true) {
                        if(mListener!=null) mListener.onEvent();
                        GlobalVariables.sendAttachment = false;
                    }
        ...
        protected Void doInBackground(Void... arg0) {
            try {
                ...
                Log.e(TAG, "Next dump is the stack trace....");
                e.printStackTrace();
                outputFile = null;
